...
Cresce além das 11 etapas da v0.1.0: de 17 para **36 bibliotecas** em `src/` e de 375 para
**726 testes** (22 projetos de teste). Build limpo (0 warnings).
...
Primeira release pública. Cobre as 11 etapas do roteiro completo de construção.
...
- **17 projetos publicáveis** (Core + Expressions + DataSources + Layout +
  Rendering + Rendering.Skia + Rendering.Gdi + CodeFirst + Serialization +
  Output.Pdf + Output.Excel + Printing + Printing.WindowsSpooler +
  Printing.EscPos + Printing.Android + Viewer.Blazor + Designer.Blazor +
  Hosting.AspNetCore).
- **4 samples**: CodeFirst (console), WindowsPrinting (console), BlazorServer
  (web), MauiHybrid (desktop+mobile).
- **375 testes** (todos verdes) — xUnit + FluentAssertions + bUnit + PdfPig +
  ClosedXML readback.
- **Cobertura ≥ 80%** em Core, Expressions, DataSources, Layout, Rendering,
  Rendering.Skia, CodeFirst, Serialization, Output.Pdf, Output.Excel,
  Rendering.Gdi, Printing.EscPos.
