...
/// actually IMPLEMENTED today: Detail PageBreak.Start / StartAndEnd, PageHeader.PrintOnFirstPage
/// gating, element-level CanShrink/CanGrow, NoRowsMessage ordering, zero margins, landscape +
/// multi-column width, continuous paper growth, and snake column flow.
///
/// All assertions were derived from the paginator + BandRenderer source, not from "ideal" RDL
/// semantics. Known limitations (band split, PrintOnLastPage, continuous + columns) live in the
/// sibling <c>PaginationLimitationCharacterizationTests</c>.
/// </summary>
public class PaginationEdgeCaseTests
{
    ...
    public async Task CanShrink_textbox_with_short_content_emits_a_shorter_primitive_than_without()
    {
        ...
        //
        // NOTE on scope: CanShrink only affects the ELEMENT primitive height. The paginator's
        // band-height accounting (MaxHeight only grows) does NOT pull the next band up — that
        // limitation is locked in PaginationLimitationCharacterizationTests.
